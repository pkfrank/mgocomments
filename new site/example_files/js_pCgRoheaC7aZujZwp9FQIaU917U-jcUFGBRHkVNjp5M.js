...
(function ($, Drupal) {
    'use strict';
    Drupal.behaviors.mgoblog_image_replace = {
        attach: function(context, settings) {

            var imagesToBeReplaced = $('[data-image-replace]');

            if (imagesToBeReplaced.length > 0) {
                imagesToBeReplaced.each(function(index, el) {
                    var imageUrl = $(el).data('image-replace');
...
(function ($, Drupal) {
    'use strict';
    Drupal.behaviors.mgoblog_global = {
        attach: function(context, settings) {

            var points = $('.field--name-points');

            if (points.length > 0) {
                points.each(function(index, el) {
                    // The points module returns points as floats, but
                    // we want them as integers so we make that adjustment
                    // here
                    var pointsInt = parseInt($(el).text()).toFixed();
                    $(el).text(pointsInt);
                });
            }
        }
...
(function ($, Drupal) {
    'use strict';
    Drupal.behaviors.mgoblog_responsive_menu = {
        attach: function(context, settings) {

            /*Handles mobile nav hamburger menu*/
            $('.menu-toggle-target').click(
               function(){
                   $('.clearfix.menu').toggleClass('open');
            });

            /*Handles top level navigation toggling between categories*/
            $('.menu-item--expanded > a').click(function(event){
                event.preventDefault();
            });

            $(".menu-item--expanded").click(function () {
                $('.menu-item--expanded > ul').not($(this).children(".menu").toggle()).hide();
            });

            /*This handles the user account and search menu display for mobile*/
            $('.account-open').click(
                function(){
                    $('.block-user-login-block').toggleClass('open');
            ...
            var stickyLogin = function(){
                var getPosition = $(window).scrollTop();
                var initTop = 145;
                var setPosition = function(){
                    var initTop = 140;
                    var getPosition = $(window).scrollTop();
                    if (getPosition === 0){
                        $('.block-user-login-block').css("top", initTop);
                    }
                    else {
                        if (getPosition < 90){
                            var newPosition = initTop - getPosition;
                            $('.block-user-login-block.open').css("top", newPosition);
                        } else if (getPosition > 90){
                            $('.block-user-login-block').css("top", 50);
                        }
                    }
                };
                setPosition();
...
/**
 * @file
 * Up/down vote icons behavior.
 */
(function ($, Drupal) {

    'use strict';

    Drupal.behaviors.mgo_vote = {
        attach: function(context, settings) {
            $('.mgo-vote-wrapper', context).once('mgo-vote').each(function () {

                var $widget = $(this);

                $widget.find('.mgo-vote__upvote a').click(function(e) {
                    var entity_id, entity_type;
                    if (!$(this).hasClass('disable-status')) {
                        e.preventDefault();
                        entity_id = $(this).data('entity-id');
                        entity_type = $(this).data('entity-type');
                        mGoVoteService.vote(entity_type, entity_id, 'upvote');
                    }
                });

                $widget.find('.mgo-vote__downvote a').click(function(e) {
                    var entity_id, entity_type;
                    if (!$(this).hasClass('disable-status')) {
                        e.preventDefault();
                        entity_id = $(this).data('entity-id');
                        entity_type = $(this).data('entity-type');
                        mGoVoteService.vote(entity_type, entity_id, 'downvote');
                    }
                });
            });
        }
...
(function ($, Drupal) {
    ...
    window.mGoVoteService = window.mGoVoteService || (function() {
            function mGoVoteService() {}
            mGoVoteService.vote = function(entity_type, entity_id, vote_type) {
                $.ajax({
                    type: "GET",
                    url: drupalSettings.path.baseUrl + 'mgo_vote/' + entity_type + '/' + entity_id + '/' + vote_type,
                    success: function(response) {

                        // Expected response is a json object where upvotes is the new number
                        // of upvotes, downvotes is the new number of downvotes, message_type is
                        // the type of message to display ("status" or "warning") and message
                        // is the message to display.

                        ['upvote', 'downvote'].forEach(function (voteType) {

                            var selector = '.mgo-vote__' + voteType + '--' + entity_type + '--' + entity_id;
                            var countWrapper = '.mgo-vote__count--' + entity_type + '--' + entity_id;
                            var voteCount = response['upvotes'] - response['downvotes'];
                            var $aTag = $(selector + ' > a');
                            ...
                            response.operation[voteType] ? $aTag.addClass('voted') : $aTag.removeClass('voted');

                            $(countWrapper + ' .count').text(voteCount);
                        });
                    }
                });
            };
            return mGoVoteService;
        })();
...
(function ($, Drupal) {
    'use strict';
    Drupal.behaviors.mgoblog_collapse_expand = {
        attach: function(context, settings) {
            $('.comment__time .collapse').once().on('click', function(){
                $(this).toggleClass('open');
                var isTopLevelComment = $(this).closest('.child').length === 0;
                if( isTopLevelComment ) {
                    $(this).closest('.thread-wrapper').toggleClass('collapsed');
                } else {
                    $(this).closest('.child').toggleClass('collapsed');
                }
            });
        }
    };
